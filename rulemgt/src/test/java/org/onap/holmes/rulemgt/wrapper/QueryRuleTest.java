...
public class QueryRuleTest {
    ...
    public void setUp() {
        ...
        queryRule = PowerMock.createMock(QueryRule.class);
    }

    @Test
    public void queryRuleByEnable() throws Exception{
        int enable = 0;
        EasyMock.expect(queryRule.queryRuleByEnable(EasyMock.anyInt())).andReturn(new ArrayList<CorrelationRule>());
        PowerMock.replayAll();
        List<CorrelationRule> correlationRules = queryRule.queryRuleByEnable(enable);
        PowerMock.verifyAll();
    ...
    public void queryRuleByEngineInstance() throws Exception{
        ...
        EasyMock.expect(queryRule.queryRuleByEngineInstance(EasyMock.anyObject(String.class))).andReturn(new ArrayList<CorrelationRule>());
        PowerMock.replayAll();
        List<CorrelationRule> correlationRules = queryRule.queryRuleByEngineInstance(engineInstance);
        PowerMock.verifyAll();
        Assert.assertThat(correlationRules, IsNull.<List<CorrelationRule>>notNullValue());
    }
}
