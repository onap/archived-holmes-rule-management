...
import org.onap.holmes.rulemgt.msb.GetEngineIp;
import org.onap.holmes.rulemgt.wrapper.QueryRule;
...
public class AllocateRule {
    ...
    @Inject
    private GetEngineIp getEngineIp;
    @Inject
    ...
    public void initDaoUtilAndEngineIp() throws Exception{
        ...
        temIpList =  getEngineIp.getServiceCount();
    }
    ...
    private List<CorrelationRule> calculateRule(List<String> oldIpList) throws Exception{
        allRules = queryRule.queryRuleByEnable(ENABLE);
        if(allRules != null) {
        ...
        for(String ip : oldIpList) {
            List<CorrelationRule> rules = queryRule.queryRuleByEngineInstance(ip);
        ...
        try{
            if(destroyIpList != null){
                for(String ip : destroyIpList) {
                    rules.addAll(queryRule.queryRuleByEngineInstance(ip));
                }
            }
    ...
    private List<String > getSortIp(List<String> ipList){
        ...
        try{
            for(String ip : ipList){
                ipRuleList = queryRule.queryRuleByEngineInstance(ip);
                if(ipRuleList != null) {
